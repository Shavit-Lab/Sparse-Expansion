...
class SparseGPT:
    ...
    def fasterprune(
        self, sparsity, prunen=0, prunem=0, blocksize=128, percdamp=0.0
    ):
        if prunen != 0:
            assert (
                self.mode == "default"
            ), "NM pruning only supported in default mode"
        if (
            self.mode == "2:4+default"
            or self.mode == "2:4+SSPN"
            or self.mode == "2:4+DSPN"
        ):
            assert (
                sparsity >= 0.5
            ), "Sparsity must be greater than 0.5 for 2:4+ mode"
            prunen = 2
            prunem = 4
                ...
                if mask is not None:
                    ...
                    if self.mode == "default":
                        ...
                        indices = torch.tensor(
                            tmp.shape[1] * sparsity, dtype=torch.int64
                        ).to(self.dev)
                        # convert indices, an np array, to a tensor with int64 dtype
                        selected = torch.gather(
                            thresh, 1, indices.unsqueeze(1)
                        ).squeeze()
